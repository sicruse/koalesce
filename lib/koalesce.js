...
var isFunction = function (fn) {
    return fn instanceof Function && !isAsyncFunction(fn) && !isGeneratorFunction(fn);
}
...
Koalesce.prototype._prepareMetadataFunc = function (route) {
    return function* () {
        let context = {};
        context.route = route;
        route._composedMetadata(context);
...
Koalesce.prototype._initializeRoute = function (file, controller, routeName, route) {
    ...
    let callStack = [convert(function* (next) {
    ...
    callStack.push(convert(route.handler));
        ...
        try {
            if ( Array.isArray(object.middleware) ) {
                for ( let i = 0 ; i < object.middleware.length ; i++ ) {
                    result.push(convert(object.middleware[i]));
                }
            } else { // it's a generator function
                result.push(convert(object.middleware));
            }
        } catch (err) {
