...
function* Koalesce (config, logInfo, logWarning, logError) {
    ...
    yield* validation.validateConfiguration(config);
...
Koalesce.prototype._loadMiddleware = function () {
    ...
    this._routeAgnosticMiddlewareConfig = this._loadMiddlewareSet(this.config.middleware.routeAgnostic);
    this._routeAwareMiddlewareConfig = this._loadMiddlewareSet(this.config.middleware.routeAware);
};
...
Koalesce.prototype._loadMiddlewareSet = function (middleware) {
    for ( let middlewareIndex in middleware ) {
        ...
        if ( object.module ) {
            _.extend(object, object.module);
            object.isModule = true;
            delete object.module;
            middleware[middlewareIndex] = object;
        }
    }
    ...
    for ( let i = 0 ; i < config.controllerPaths.length ; i++ ) {
        ...
        for ( let fileIndex in files ) {
            ...
            if ( fileStat.isFile() ) {
                ...
                let errors = yield* validation.validateController(filePath, controller.instance);
...
Koalesce.prototype._initializeMiddleware = function () {
    ...
    this._routeAgnosticMiddleware = this._initializeMiddlewareSet(this._routeAgnosticMiddlewareConfig); 
